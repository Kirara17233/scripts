...
if [ $model -eq 1 ];then
  ...
  yay -S --noconfirm xf86-video-vmware open-vm-tools alsa-utils numlockx gobject-introspection nix\
      ...
      gnome-keyring ttf-meslo-nerd-font-powerlevel10k ttf-jetbrains-mono noto-fonts noto-fonts-sc noto-fonts-emoji jdk-openjdk jetbrains-toolbox visual-studio-code-bin google-chrome bash-pipes cmatrix
  mkdir ~/Desktop
EOF
"
